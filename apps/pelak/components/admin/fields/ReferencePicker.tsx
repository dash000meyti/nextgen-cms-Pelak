...
type ReferencePickerProps = {
  ...
  multiple?: boolean;
  readOnly?: boolean;
  /** Hide the static label; placeholder / floating label use `label` text. */
  labelAsPlaceholder?: boolean;
  /** Anchor for form feedback scroll (`data-field`). */
  fieldKey?: string;
};
...
export function ReferencePicker({
  ...
  const floated =
    labelAsPlaceholder &&
    (selected.length > 0 || open || query.trim().length > 0);

  const inputPlaceholder = labelAsPlaceholder
    ? undefined
    : selected.length === 0
      ? "جستجو…"
      : "افزودن…";
    ...
    >
      {selected.map((option) => (
        <span
          ...
          className={
            readOnly
              ? "rounded bg-paper px-2 py-0.5 text-xs text-ink ring-1 ring-rule"
              : "inline-flex max-w-full items-center gap-1 rounded bg-accent-soft px-2 py-0.5 text-xs text-accent"
          }
        >
      ...
      {readOnly ? (
        selected.length === 0 && !labelAsPlaceholder ? (
  ...
  return (
    <div ref={rootRef} className="relative space-y-1.5" data-field={fieldKey}>
      ...
      {labelAsPlaceholder ? (
        <div className="relative">
          <FloatLabel text={label} floated={floated} />
          {shell}
        </div>
      ) : (
        shell
      ...
      {!readOnly && open ? (
        ...
        >
          {filtered.length === 0 ? (
            <p className="px-2 py-2 text-sm text-ink-muted">موردی یافت نشد.</p>
          ) : (
            filtered.map((option) => {
