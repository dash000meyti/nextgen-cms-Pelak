...
- فرم محتوا (`ArticleForm`) — متادیتا فشرده (بدون بدنه):
...
- دانلود از UI مستقیماً از `pdfSrc` (uploads) انجام می‌شود.
- نام دانلودی فایل آپلودشده برای گروه محتوا به `title.pdf` ست می‌شود.
- لینک دانلود فقط وقتی نمایش داده می‌شود که `pdfSrc` وجود داشته باشد.
