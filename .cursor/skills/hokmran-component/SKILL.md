...
- `AdminLink` در `layout/`: Server Component؛ فقط وقتی `SiteHeader` با `showAdminLink` رندر شود — session در `app/layout.tsx` خوانده می‌شود.
- primitives موجود را بسط بده: `Tag`, `Button`, `AuthorChipList`, `Container`, `FormMessage`, …
- فرم‌ها: `useFormFeedback` + `data-field` روی فیلدها — `docs/UI-BOUNDARY.md`
